...
use deluge_bsp::usb::{Rusb1Driver, dcd_int_handler, init_device_mode};
use rza1l_hal::{allocator, gic, rusb1};
...
/// Set on first entry to [`panic`] to prevent recursive panics.
static PANICKING: AtomicBool = AtomicBool::new(false);

#[panic_handler]
fn panic(info: &PanicInfo) -> ! {
    // The global logger borrows an rtt-target `RefCell`. If the original panic
    // happened while that borrow was held (e.g. mid-log-write), logging here
    // re-borrows → double-panics → recurses until the stack overflows. The guard
    // limits us to one logging attempt, then spins — so a single panic can never
    // wedge the chip. In the common case (panic outside a log write) the message
    // still prints; otherwise we land here cleanly for a debugger backtrace.
...
/// drained by `midi_tx_task`.
static MIDI_TX: Mutex<CriticalSectionRawMutex, RefCell<ByteRing<256>>> =
    Mutex::new(RefCell::new(ByteRing::new()));

// ── UI (pads/buttons/encoders in; LEDs/OLED out) ─────────────────────────────

/// PIC input events (`pic_task` → `vm_task`), 2 bytes each: `[kind, id]`
/// (kind: 0=pad press, 1=pad release, 2=button press, 3=button release).
...
/// Set by `pic_task` once the PIC + OLED are initialised (gates `ui_task`).
static PIC_READY: AtomicBool = AtomicBool::new(false);
...
/// Push a PIC input event for `vm_task` to dispatch.
fn input_push(kind: u8, id: u8) {
    INPUT_EVENTS.lock(|r| r.borrow_mut().push(&[kind, id]));
}
...
pub extern "C" fn main() -> ! {
    ...
    info!("platform: clocks, MMU, cache, SDRAM, GIC, OSTM ready");

    // CV DAC (MAX5136 on RSPI0) + gate GPIO for the Output/Gate bindings.
    unsafe { deluge_bsp::cv_gate::init() };
    info!("cv_gate: MAX5136 CV DAC + gates ready");

    // MIDI DIN (SCIF0 @ 31250 bps, DMA RX) for the Midi bindings.
    unsafe { deluge_bsp::uart::init_midi(31_250) };
    info!("midi: DIN UART ready (SCIF0 @ 31250)");

    // PIC32 UART (SCIF1) for pads/buttons/LEDs/OLED; pic::init() handshakes the
    // baud rate up in pic_task.
    unsafe { deluge_bsp::uart::init_pic(31_250) };

    // Audio: SSI0 → WM8962 codec, direct (SCUX bypassed). The audio_task renders
    // the DSP graph into the SSI TX buffer at 44.1 kHz.
    unsafe { deluge_bsp::audio::init_direct() };
    info!("audio: SSI0 direct path ready (44.1 kHz, SCUX bypassed)");

    // Boot the VM *before* the USB pull-up so the device only enumerates once
    // the VM is up (mirrors crow's ordering; wren_boot is ~0.6 s).
    ...
    // Load the M2 prelude: declares the Output/Gate/Metro foreign classes and
    // the `output[]` / `gate[]` accessors in the `main` module.
    info!("wren: loading prelude...");
    run_source(vm, bindings::prelude_ptr(), "prelude");

    // USB0 device-mode interrupt.
    unsafe {
        gic::register(rusb1::USB0_IRQ, || dcd_int_handler(0));
    }

    // Build the USB-CDC-ACM device.
    ...
    // Quadrature encoders (GPIO IRQs) — must be set up before global IRQ enable.
    ...
    executor.run(|spawner: Spawner| {
        ...
        spawner.spawn(midi_tx_task().unwrap());
        spawner.spawn(pic_task().unwrap());
        spawner.spawn(ui_task().unwrap());
        spawner.spawn(audio::audio_task().unwrap());
    });
}
...
/// SD-card script persistence. Reads `/MAIN.WREN` at boot (staged for `vm_task`
/// to run), and writes it back on `^^w`. Best-effort: a missing/unformatted card
/// just disables persistence.
#[embassy_executor::task]
async fn flash_task() {
    use deluge_bsp::fat::{self, Mode, VolumeIdx};
    use deluge_bsp::sd;

    if let Err(e) = sd::init().await {
        warn!("flash: SD init failed ({:?}); persistence disabled", e);
        return;
    }
    let fs = fat::new_volume_manager();
    let volume = match fs.open_raw_volume(VolumeIdx(0)) {
        Ok(v) => v,
        Err(_) => {
            warn!("flash: no FAT volume; persistence disabled");
            return;
        }
    };
    let root = match fs.open_root_dir(volume) {
        Ok(d) => d,
        Err(_) => {
            warn!("flash: cannot open root dir; persistence disabled");
            return;
        }
    };

    // ── Boot load: read /MAIN.WREN, stage it for vm_task ──
    match fs.open_file_in_dir(root, "MAIN.WREN", Mode::ReadOnly) {
        Ok(file) => {
            // SAFETY: BOOT_BUF is written only here, before BOOT_PENDING is set.
            let buf = unsafe { &mut *addr_of_mut!(BOOT_BUF) };
            let mut len = 0usize;
            while len < SCRIPT_MAX {
                match fs.read(file, &mut buf[len..SCRIPT_MAX]) {
                    Ok(0) => break,
                    Ok(n) => len += n,
                    Err(_) => break,
                }
    ...
    loop {
        ...
        let data = &save_buf[..n];
        match fs.open_file_in_dir(root, "MAIN.WREN", Mode::ReadWriteCreateOrTruncate) {
            Ok(file) => {
                let ok = fs.write(file, data).is_ok();
                let _ = fs.flush_file(file);
                let _ = fs.close_file(file);
                if ok {
                    info!("flash: saved MAIN.WREN ({} bytes)", n);
                } else {
                    warn!("flash: write failed");
                }
            }
            Err(_) => warn!("flash: cannot open MAIN.WREN for write"),
        }
    }
}

/// Drains the MIDI DIN TX ring to the SCIF0 UART. `write_midi` awaits FIFO
/// space, so no bytes are dropped once queued.
#[embassy_executor::task]
async fn midi_tx_task() {
    let mut buf = [0u8; 16];
    loop {
        ...
        deluge_bsp::uart::write_midi(&buf[..n]).await;
    }
}

/// PIC32 UI task: bring up the PIC + OLED, then read the PIC byte stream,
/// forwarding pad/button events into `INPUT_EVENTS` (dispatched by vm_task) and
/// the OLED CS-handshake events to the BSP. (Mirrors demo-firmware's pic_task.)
#[embassy_executor::task]
async fn pic_task() {
    deluge_bsp::pic::init().await;
    deluge_bsp::oled::init().await;
    PIC_READY.store(true, Ordering::Release);
    info!("pic: UI ready (pads/buttons/LEDs/OLED)");

    let mut parser = deluge_bsp::pic::Parser::new();
    loop {
        let byte = rza1l_hal::uart::read_byte(deluge_bsp::pic::UART_CH).await;
        let Some(event) = parser.push(byte) else {
            continue;
        };
        use deluge_bsp::pic::Event;
        match event {
            Event::PadPress { id } => input_push(0, id),
            Event::PadRelease { id } => input_push(1, id),
            Event::ButtonPress { id } => input_push(2, id),
            Event::ButtonRelease { id } => input_push(3, id),
            // OLED CS handshake — must be forwarded for send_frame to complete.
            Event::OledSelected => deluge_bsp::pic::notify_oled_selected(),
            Event::OledDeselected => deluge_bsp::pic::notify_oled_deselected(),
            _ => {}
        }
    }
}

/// UI output task: drains LED commands and re-renders the OLED. Both PIC and
/// OLED operations are async, so they live here rather than in a foreign method.
#[embassy_executor::task]
async fn ui_task() {
    // Wait for pic_task to finish PIC + OLED init before issuing commands.
    while !PIC_READY.load(Ordering::Acquire) {
    ...
    loop {
        ...
        loop {
            ...
            if led[1] != 0 {
                deluge_bsp::pic::led_on(led[0]).await;
            } else {
                deluge_bsp::pic::led_off(led[0]).await;
            }
        }
        // OLED re-render (copy the frame buffer out, then DMA it).
        if OLED_SHOW.swap(false, Ordering::Acquire) {
            let fb = OLED_FB.lock(|r| r.borrow().clone());
            deluge_bsp::oled::send_frame(&fb).await;
        }
        Timer::after(Duration::from_millis(5)).await;
    }
}
...
async fn vm_task() {
    ...
    let mut enc_pos = [0i8; deluge_bsp::encoder::NUM_ENCODERS];

    loop {
        // Render tick: advance CV slew, push DAC/gates, fire due metro callbacks.
        ...
        // MIDI DIN RX: parse bytes and dispatch to wren callbacks. Runs here (the
        // VM-owning task) so it never re-enters wren mid-evaluation.
        while let Some(b) = deluge_bsp::uart::try_read_midi() {
            if let Some((s, d1, d2)) = midi.push(b) {
                bindings::midi_rx(vmw, s, d1, d2);
            }
        }

        // PIC input events (pads/buttons) → wren callbacks.
        let mut ev = [0u8; 2];
        loop {
            ...
            bindings::input_dispatch(vmw, ev[0], ev[1]);
        }

        // Encoder detents → wren callbacks.
        for i in 0..deluge_bsp::encoder::NUM_ENCODERS {
            let d = deluge_bsp::encoder::take_detents(i, &mut enc_pos[i]);
            if d != 0 {
                bindings::enc_turn(vmw, i as u8, d);
            }
        }
